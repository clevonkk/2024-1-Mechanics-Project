...
# [ Reading Input.txt & Setting Up Variables ]
# This section reads parameters from Input.txt file.

with open("Input.txt", "r") as f:
    input1_line = f.readline()
    input1_list = input1_line.split()

    input2_line = f.readline()
    input2_list = input2_line.split()

    input3_line = f.readline()
    input3_list = input3_line.split()
    mode = int(input3_list[0])

    if mode == 1:
        input4_line = f.readline()
        input4_list = input4_line.split()
        num = int(input4_list[0])

        tlist = []
        for i in range(num):
            tlist.append(float(f.readline()))
        t_i = tlist[0]
        t_f = tlist[num-1]
        ...
        input4_list = input4_line.split()
        t_i = 0.00
        t_f, dt = list(map(float, input4_list))
        delt = dt
...
if mode == 1:
    ...
    Mode 1 writes a text file "Output.txt". Check your folder of
    same directory of this python program.

    The text file includes (i) theta1, (ii) omega1, (iii) theta2,
    and (iv) omega2 for times in tlist. All values are given in
    *float* type, to the accuracy of thousandths place value.
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    ...
    for i in range(len(tlist)):
        ...
        res = np.array2string(data, separator = ' ', formatter = {'float_kind':lambda x: "%.3f" % x})
        f.write(res.lstrip('[').rstrip(']'))
        f.write("\n")
    f.close()
